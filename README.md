...
> 建议将以下占位图替换为项目真实截图，方便新成员和使用者快速理解系统能力。

### 1. 桌面端主界面

![桌面端主界面占位图](docs/images/desktop-main.png)

### 2. 工单详情界面

![工单详情占位图](docs/images/ticket-detail.png)

### 3. Web 看板总览

![Web 看板占位图](docs/images/dashboard-overview.png)
...
建议按以下顺序联调，避免“服务都启动了但数据不通”的问题。

1. 启动 MySQL 并确认可连接。
2. 导入 `sql/mcs_by_takuya.sql` 并检查核心表已创建。
3. 完成 `src/core/config.py` 数据库配置。
4. 启动桌面端：`python main.py`。
5. 检查看板端连接同一数据库实例。
6. 执行一条端到端流程：创建工单 -> 状态流转 -> 完成 -> 看板统计更新。
